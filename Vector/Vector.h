 ...
 *
 * This header defines the `VHeader_` structure, along with functions and macros for dynamically managing vectors.
 * A vector is a dynamic array that automatically grows in size when more elements are added. The vector functions
 * handle dynamic memory allocation, resizing, and deallocation efficiently. The `Allocator` structure is used to
 * manage memory for vectors, and custom allocators can be set using `vec_set_allocator`.
 *
 * @version 1.0
 * @date 12.06.2025
 *
 ...
 *
 * This structure stores the metadata for a vector, including its allocator, current size, and capacity.
 * It is used internally to manage the vector's dynamic memory allocation and resizing.
 */
typedef struct VHeader_
{
    ...
    size_t capacity;     ///< The total capacity of the vector (number of elements it can store).
} VHeader_;

extern Allocator Current_vector_allocator; /**< The global allocator used for vectors */

/**
 * @brief Retrieves the header of the vector.
 *
 * This macro calculates the address of the vector header by using pointer arithmetic.
 * The header is located just before the actual vector data.
 *
 ...
 *
 * This macro retrieves the allocator used to allocate memory for the vector. If the vector is `NULL`,
 * it uses the default allocator `Current_vector_allocator`.
 *
 * @param vec The pointer to the vector.
 ...
 *
 * This macro frees the memory allocated for the vector, including its header and data.
 *
 ...
 *
 * This macro provides a way to iterate over a vector. It can be customized with a start index and an end index.
 *
 ...
 *
 * This function resets the allocator to the default allocator (`malloc`), which is used for vector memory allocations.
 */
 ...
 *
 * This function deallocates the memory used by the vector, including the header and data.
 *
 ...
 *
 * This function returns the current capacity (total number of elements the vector can hold).
 *
 * @param vec The pointer to the vector.
 ...
 *
 * This function resets the vector size to zero, effectively clearing all elements in the vector.
 *
 ...
 *
 * This function allocates memory for a new vector with the specified element size and initial capacity.
 *
 ...
 *
 * This function reallocates the vector to a larger size if it is full, allowing more elements to be added.
 * If the vector is `NULL`, it is safely reallocated.
 *
 * @param vec The pointer to the vector.
 * @param elem_size The size of each element in the vector.
 ...
 *
 * This macro appends a value to the vector. If the vector is full, it is reallocated to accommodate the new element.
 * If the vector is `NULL`, it is reallocated safely.
 *
 * @param vec The pointer to the vector.
 ...
 *
 * This macro removes and returns the last element of the vector. If the vector is empty, it returns an empty value.
 *
 * @param vec The pointer to the vector.
 ...
 *
 * This macro reallocates the vector to the specified new capacity, retaining its current elements.
 * If the vector is `NULL`, it is safely reallocated.
 *
 * @param vec The pointer to the vector.
