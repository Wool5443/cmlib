 ...
 *
 * This header defines the data structures and functions required to implement a free list-based
 * memory allocator. The allocator uses multiple memory pools to manage blocks of memory, allowing for
 * efficient allocation and deallocation of memory in a system with potentially varying allocation sizes.
 * It provides functions for memory pool creation, memory allocation, and memory freeing, ensuring that
 * blocks of memory are reused efficiently to reduce fragmentation.
 *
 * The allocator works by managing free blocks of memory using a linked list structure. When a memory block
 * is allocated, the corresponding free block is removed from the list. When memory is freed, it is added back
 ...
 *
 * This structure holds the size of the block and a pointer to the next free block in the list.
 */
 ...
 *
 * This structure holds a pointer to the next pool, a pointer to the next free block in the pool,
 * and a pointer to the end of the pool.
 */
...
struct Free_list_memory_pool
{
    ...
    Free_list_free_block_header* next_block; /**< Pointer to the next free block in the pool */
 ...
 *
 * This function creates and initializes the Free List allocator with a specified pool size.
 *
 * @param pool_size Size of the memory pool to be created.
 * @return A result containing either the successfully created allocator or an error code.
 */
 ...
 *
 * This function deallocates all memory pools managed by the Free List allocator.
 *
 ...
 *
 * This function allocates a block of memory from the Free List allocator. It first tries to allocate
 * from the existing memory pools. If no suitable free block is available, a new memory pool is created.
 *
 ...
 *
 * This function frees a block of memory previously allocated by the Free List allocator. It adds the block
 ...
 *
 * This function allocates a block of memory from the specified memory pool. If no suitable free block is
 ...
 *
 * This function checks if a pointer lies within the valid range of a memory pool.
 *
 * @param pool Pointer to the memory pool.
 ...
 *
 * This function frees a previously allocated memory block and returns it to the pool of free blocks.
 *
 * @param pool Pointer to the memory pool.
 * @param ptr Pointer to the memory block to be freed.
