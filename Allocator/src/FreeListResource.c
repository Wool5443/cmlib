...
static void* free_list_resource_allocate(MemoryResource* resource,
    size_t size,
    size_t alignment)
{
    if (!resource)
...
static void free_list_resource_deallocate(MemoryResource* resource, void* ptr)
{
    if (!resource)
    {
        return;
...
Result_FreeListResource free_list_resource_ctor(size_t pool_size)
{
    ...
    return Result_FreeListResource_ctor(
        free_list_to_resource(&free_list.value),
        EVERYTHING_FINE);
}
